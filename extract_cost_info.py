...
total_queries = len(all_extracted_data)
total_cost = sum(
    item["cost"] for item in all_extracted_data if item["cost"] is not None
)
average_cost = total_cost / total_queries if total_queries > 0 else 0

totals = {
    "tokensIn": sum(
        item["tokensIn"] for item in all_extracted_data if item["tokensIn"] is not None
    ),
    "tokensOut": sum(
        item["tokensOut"]
        for item in all_extracted_data
        if item["tokensOut"] is not None
    ),
    "cacheWrites": sum(
        item["cacheWrites"]
        for item in all_extracted_data
        if item["cacheWrites"] is not None
    ),
    "cacheReads": sum(
        item["cacheReads"]
        for item in all_extracted_data
        if item["cacheReads"] is not None
    ),
    "cost": total_cost,
    "totalQueries": total_queries,
    "averageCostPerQuery": average_cost,
}
